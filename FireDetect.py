        ...
        for r in range(rows):
            ...
            if confidence >= 0.6:
                ...
                if (classes_scores[class_id] > .25):
                    ...
                    bot = int((y + 0.5 * h) * y_factor)
                    #width = int(w * x_factor)
                    #height = int(h * y_factor)
                    box = np.array([left, right, top, bot])
                    boxes.append(box)
...
while(True):
	 ...
	 #For Printing out Fire
        for (classid, confidence, box) in zip(class_ids, confidences, boxes):
                ...
                cv2.rectangle(frame, (box[0],box[2]),(box[1],box[3]), color, 2)
         	#cv2.rectangle(frame, (box[0], box[2] - 20), (box[1], box[3]), color, -1)
                cv2.putText(frame, "{}: {:.4f}".format('fire', confidence), (box[0], box[2] - 10), cv2.FONT_HERSHEY_SIMPLEX, .5, (180,40,50),2)
                print(confidence)
